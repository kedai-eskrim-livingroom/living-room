...
function GrowthChart({ data }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H + 24}`} className="w-full mt-2" style={{ overflow: "visible" }}>
      ...
      {data.map((d, i) => (
        <text key={i} x={pts[i].x} y={H+20} textAnchor="middle" fontSize="11" fill="#A89A8E" fontFamily="Inter,sans-serif">{d.label}</text>
      ))}
    </svg>
  );
}
...
export default function Dashboard() {
  ...
  const growthData = d.growth || d.growthData || [];
